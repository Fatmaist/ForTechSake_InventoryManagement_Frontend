...
export default function Homepage() {
  ...
  const handleDelete = async (id_restock) => {
    try {
      ...
      const updatedRestocks = restocks.filter(
        (restock) => restock.id_restock !== id_restock
      );
      ...
      <Table variant="simple" w="70%" p={6} marginRight={-300}>
        ...
        <Tbody>
          {filteredRestocks.slice(startIndex, startIndex + 5).map((restock) => (
            <Tr
              key={restock.id_restock}
              onContextMenu={(e) => handleContextMenu(e, restock)}
            >
              ...
              <Td>
                <Button
                  colorScheme="blue"
                  size="sm"
                  onClick={() => handleEdit(restock.id_restock)}
                >
                  <Link href="">Edit</Link>
                </Button>
              </Td>
              <Td>
                <Menu>
                  ...
                  <MenuList>
                    <MenuItem onClick={() => handleDelete(restock.id_restock)}>
                      Hapus
                    </MenuItem>
                  </MenuList>
                </Menu>
